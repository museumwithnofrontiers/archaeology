...
export default {
  ...
  // addresses every DXA config repeats, archaeology adding none of its own.
  links: { ...mwnfLinks },

  // The source-database chip's colour, per project (epic #1727 phase 4):
  // one entry for every project id `npm pack @museumwnf/archaeology-data`
  // carries in `manifest.projects` (checked 2026-09-22, archaeology-data
  ...
  // (`manifest.projects[id].name.en`). Archaeology owns no native items —
  // every project below is a borrowed one, so there is no "this gallery's
  // own" entry the way carpets' Discover Carpet Art is its own.
  projectColors: {
    ...
    'dcf7b4d2-03c8-568a-8209-2817950fe05e': 'mwnf-chip--DCA', // Discover Carpet Art — borrowed, carpets' own project
  ...
  // that borrows from Explore Islamic Art Collections shares this list by
  // construction.
  ...
  site: { origin: 'https://museumwithnofrontiers.github.io/archaeology' },
  ...
  extraViews: [
    ...
    // results/gallery) is byte-identical to every other DXA gallery's own
    ...
    // `body` entry name).
    ...standardRoutes('gallery', { creditsBody: 'archaeology.credits.body' }),
  ],
