...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('archaeology.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('archaeology.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('archaeology.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
