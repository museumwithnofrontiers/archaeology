...
The **Museum With No Frontiers — Archaeological Objects** gallery (legacy env
key `arc`): a gallery built entirely from borrowed material — it holds no
...
Islamic Art Collections, Sharing History, MWNF Galleries, Discover Glass Art,
Discover Carpet Art, The Table Is Set, The Hijaz Railway) — the collection,
its dependent filters, a full-text database search, partner profiles and
their holdings, country timelines and the item sheets themselves, all built
...
| Package | Role |
| --- | --- |
| `@museumwnf/archaeology-data` | the dataset (JSON + `manifest.json`) |
...
overloaded there with the Archaeology text (it explains, among other things,
that this gallery holds no native collection of its own), while
`archaeology.credits.body` is a name of this gallery's own, because there is
no generic credits page to inherit.
...
<https://museumwithnofrontiers.github.io/archaeology/> through the reusable workflows in
