...
describe('website smoke test', () => {
  ...
  it('renders the collection results on the composed results view', async () => {
    ...
    // Nine a page, more than one page (151 items in the package).
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain('Archaeological Objects')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    ...
    const isl = await mountSite('#/item/9fd48483-773e-56ef-a4ac-80ac9b4f89d0')
  ...
  // for Discover Islamic Art, so this asserts the positive case against the
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '9fd48483-773e-56ef-a4ac-80ac9b4f89d0')
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    // An *Ataifor* (bowl) with the Umayyads of al-Andalus dynasty attached —
    // a record whose dynasty translation actually carries a history, so it
  ...
  it('renders the timeline results on the composed timeline view', async () => {
    ...
    expect(host.querySelector('.mwnf-summary').textContent).toContain('22')
    // An Austria event's own description, not just a row count — its
    // translation loads asynchronously, behind the spec's own `tr`.
    await vi.waitFor(() => expect(host.textContent).toContain('Solferino'), { timeout: 20000 })
    // The join to member items finds Austria's thirteen dated objects even
    // with no period chosen — "all countries too" behaviour.
    expect(host.querySelector('.mwnf-timeline__gallery').textContent).toContain('13')
    app.unmount()
  }, 60000)
  ...
  it('renders the timeline gallery on the composed results view', async () => {
    ...
    expect(host.textContent).toContain('Reserve head')
    app.unmount()
  }, 60000)

  // The composed entrance's own country control writes the inventory id
  // (`aut`), not the legacy two-letter code (`at`) the gallery's own deep
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    // a generic count.
    expect(host.textContent).toContain('13 object(s) in this site')
    app.unmount()
  }, 60000)
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['archaeology', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
